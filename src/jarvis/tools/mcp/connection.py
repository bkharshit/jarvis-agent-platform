...
An async context manager over the SDK v2 unified `Client`. Env refs resolve
from the process env at connect time; values are used, never stored. Stdio
subprocesses get the SDK's minimal default environment (PATH/HOME/…) plus
the referenced variables — never the parent's wholesale environment.
"""
...
class McpServerConnection:
    ...
    def __init__(self, server: McpServer, connect_timeout: float) -> None:
        self._server = server
    ...
    async def _build_client(self) -> Client:
        ...
        if isinstance(config, McpStdioConfig):
            # Env refs resolve HERE, from the process env — names are what
            # the DB stores, values never enter config, DB, logs, or API.
    ...
    def _resolve_env(self, config: McpStdioConfig) -> dict[str, str]:
        env: dict[str, str] = {}
        for child_key, ref in config.env.items():
            ...
            if value is None:
                raise McpResolutionError(
                    self._server.name,
                    f"environment variable {ref.env_var!r} (for {child_key!r}) is not set",
                )
            env[child_key] = value
        return env

    def _resolve_headers(self, config: McpHttpConfig) -> dict[str, str]:
        ...
        for header, ref in config.headers.items():
            ...
            if value is None:
                raise McpResolutionError(
                    self._server.name,
                    f"environment variable {ref.env_var!r} (for header {header!r}) is not set",
                )
