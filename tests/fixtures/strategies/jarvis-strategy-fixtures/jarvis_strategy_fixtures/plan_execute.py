...
- Phase PLAN (no assistant reply yet): one model invocation asking for a
  numbered plan, returned as a **think step** (empty `tool_calls` — the
  orchestrator persists the assistant message and continues the loop).
...
case-insensitive substring. Markers come from `StrategyConfig.params` via
...
class PlanExecuteStrategy:
    ...
    async def step(
        ...
        # The done marker matches as a case-insensitive substring, and phase
        # advancement is by transcript position (assistant-turn count) — live
        # models drift on marker placement ("...DONE:" at the end), casing
        # ("done"), and wording; the turn count is model-proof. Markers still
        # drive the instructions and the done check. (plan_marker stays an
        # accepted param for instruction tuning but no longer gates phases.)
        ...
        # Phase EXECUTE — the plan exists (turn 1 wrote it); a DONE reply ends
        # the run.
